...
def _auto_stats_path_from_data_dir(data_dir: str) -> Path:
    # stats/<last_dir_name>/stats.npz
    leaf = Path(data_dir).name
    return Path("stats") / leaf / "stats.npz"
...
def plot_trajs(
    ...
    *,
    ...
    if draw_boxes:
        # frame0 (history first point) box
        add_vehicle_box(ax, history_xy[0], edgecolor="gray", linewidth=1.5, alpha=0.9)
        # pred end box
        add_vehicle_box(ax, pred_future_xy[-1], edgecolor="tab:blue", linewidth=1.5, alpha=0.95)
...
def main():
    ...
    data_dir_abs = str(_resolve_path(cfg_dir, data_dir)) if data_dir else ""
    ...
    if stats_cfg.lower() in ("", "auto"):
        ...
        stats_path = _auto_stats_path_from_data_dir(data_dir_abs)
